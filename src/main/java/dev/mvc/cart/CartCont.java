...
public class CartCont {
    ...
    public ModelAndView list_by_memberno(HttpSession session,
      ...
      if (session.getAttribute("memberno") != null) { // 회원으로 로그인을 했다면 쇼핑카트로 이동
        ...
        mav.setViewName("redirect:/member/login2.do"); // /WEB-INF/views/member/login_ck_form.jsp
      }
      return mav;
    }
     ...
     * @return
     */
    @RequestMapping(value="/cart/list_by_memberno2.do", method=RequestMethod.GET )
    public ModelAndView list_by_memberno2(HttpSession session,
                                                                @RequestParam(value="cartno", defaultValue="0") int cartno ) {
      ModelAndView mav = new ModelAndView();
      ...
      if (session.getAttribute("memberno") != null) { // 회원으로 로그인을 했다면 쇼핑카트로 이동
        ...
        mav.setViewName("/cart/list_by_memberno2"); // /WEB-INF/views/categrp/list_by_memberno.jsp
        
      } else { // 회원으로 로그인하지 않았다면
        mav.addObject("return_url", "/cart/list_by_memberno2.do"); // 로그인 후 이동할 주소 ★
