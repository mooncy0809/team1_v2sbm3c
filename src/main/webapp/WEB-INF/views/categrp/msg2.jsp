...
<DIV class='message'>
  <fieldset class='fieldset_basic'>
    <UL>
      <c:choose>
        <c:when test="${code == 'create_success'}"> <%-- mav.addObject --%>
          <LI class='li_none'>
      ...
      <LI class='li_none'>
        ...
        <c:choose>
          <c:when test="${msg == 'child_record_found' }">
            ...
            카테고리를 모두 삭제해주세요.<br>
            『<A href="../cate/list_by_categrpno2.do?categrpno=${param.categrpno }">소속된 카테고리 삭제</A>』
          </c:when>
        </c:choose>
      </LI>
      <LI class='li_none'>
        ...
        <button type='button' onclick="location.href='./list2.do'" class="btn">목록</button>
      </LI>
    </UL>
  </fieldset>
